...
/**
 * IBR_OPCO300 Create API
 *
 * - Receives IBR_OPCO300Data + numRequested from request body
 * - Builds a mongoose document using conditional spreads (only saves fields that exist)
 * - Creates an IBR_OPCO300 configuration and pushes it into the authenticated user's cart
 *
 * Note:
 * technicianNote is an optional field for internal/technician remarks and does not affect processing.
 */
router.post("/", authenticate, async (req, res) => {
    // used for IBR_OPCO300 form
    try {
        ...
        const order = new IBR_OPCO300({
            ...
            ...(IBR_OPCO300Data.technicianNote && { technicianNote: IBR_OPCO300Data.technicianNote }),
