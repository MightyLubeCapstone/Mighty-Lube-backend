...
const { updateOrderID, updateOrderIDConfiguration } = require("../utils/orderutils");
...
router.put('/add-to-configurations', authenticate, async (req, res) => {
    try {
        const { oldOrderID } = req.body;
        if (!oldOrderID) return res.status(400).json({ error: 'oldOrderID is required' });

        const newOrderID = await updateOrderIDConfiguration(req.user._id, oldOrderID);
        if (!newOrderID) return res.status(404).json({ error: 'No matching configuration item found' });
