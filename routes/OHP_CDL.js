...
router.post("/", authenticate, async (req, res) => {
  // This route creates an OHP_CDL configuration object from the request payload,
  // attaches it to the authenticated user's cart, and saves it for later checkout.
  try {
    const { OHP_CDLData, numRequested } = req.body;
    ...
    const order = new OHP_CDL({
      ...
      chainSize: OHP_CDLData.chainSize,
      ...(OHP_CDLData.otherChainSize && { otherChainSize: OHP_CDLData.otherChainSize }),
      ...(OHP_CDLData.industrialChainManufacturer && {
      ...
      ...(OHP_CDLData.specialControllerOption && {
      ...
      // Added: optional technician note captured from technician/installer during form fill.
      ...(OHP_CDLData.technicianNote && { technicianNote: OHP_CDLData.technicianNote }),
    });
