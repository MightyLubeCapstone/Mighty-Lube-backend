...
/**
 * IBR_OP4OE Create API
 *
 * - Receives IBR_OP4OEData + numRequested from request body
 * - Builds a mongoose document using conditional spreads (only stores values that exist)
 * - Creates an IBR_OP4OE configuration and pushes it into the authenticated user's cart
 *
 * Note:
...
router.post("/", authenticate, async (req, res) => {
    ...
    try {
        ...
        const order = new IBR_OP4OE({
            ...
            ...(IBR_OP4OEData.technicianNote && { technicianNote: IBR_OP4OEData.technicianNote }),
