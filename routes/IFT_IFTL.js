 ...
 * IFT_IFTL Create API
 *
 * - Receives IFT_IFTLData + numRequested from request body
 * - Creates an IFT_IFTL mongoose document (stores required + optional fields)
 ...
 * It does not affect any calculations/logic; it is stored just for reference.
 */
router.post("/", authenticate, async (req, res) => {
    try {
        const { IFT_IFTLData, numRequested } = req.body;
        ...
        const order = new IFT_IFTL({
            ...
            ...(IFT_IFTLData.technicianNote && { technicianNote: IFT_IFTLData.technicianNote }),
