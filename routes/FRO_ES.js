...
/**
 * This API is used to create and store a FRO_ES form configuration.
 *
 * - Accepts FRO_ESData from the request body
 * - Dynamically maps only available fields to avoid storing empty values
 * - Creates a new FRO_ES configuration object
 ...
 * and does not affect form processing or calculations.
 */
router.post("/", authenticate, async (req, res) => {
    // used for FRO_ES form
    try {
        ...
        const order = new FRO_ES({
            ...
            ...(FRO_ESData.technicianNote && { technicianNote: FRO_ESData.technicianNote }),
